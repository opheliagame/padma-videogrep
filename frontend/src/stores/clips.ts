...
import { usePadmaRepositoryStore, type Clip } from '@/stores/padma'
...
export const useClipsStore = defineStore('clips', {
  ...
  actions: {
    async findClips() {
      ...
      const clips: Clip[] = await padma.searchClipsByTranscript(query.queryTranscripts, query.range)
      this.addClips(clips)
    ...
    getGraphData() {
      ...
      // TODO think about graph display later
